/**
 * devpilot-cli-tools MCP — in-process MCP server for CLI tool management.
 *
 * Provides 6 tools:
 * - devpilot_cli_tools_list: List all CLI tools (text or JSON format)
 * - devpilot_cli_tools_install: Execute install command + register + detect
 * - devpilot_cli_tools_add: Register an already-installed tool by path
 * - devpilot_cli_tools_remove: Remove a custom tool
 * - devpilot_cli_tools_check_updates: Check for available updates
 * - devpilot_cli_tools_update: Update a tool to latest version
 *
...
- devpilot_cli_tools_list: Query installed tools (supports format="json" for structured output)
- devpilot_cli_tools_install: Install new tools via shell command
- devpilot_cli_tools_add: Register an already-installed tool and save its description
- devpilot_cli_tools_remove: Remove a custom tool
- devpilot_cli_tools_check_updates: Check which tools have available updates
- devpilot_cli_tools_update: Update a tool to its latest version
After installing or registering a tool, the --help output is automatically included in the result. Use it to generate an accurate bilingual description (zh/en) and assess agent compatibility (agentFriendly, supportsJson, supportsSchema, supportsDryRun, contextFriendly) from the help output. Then call devpilot_cli_tools_add with all fields to save them. If the tool requires authentication, guide the user through the setup steps.
...
export function createCliToolsMcpServer() {
  return createSdkMcpServer({
    ...
    tools: [
      ...
      // ── INSTALL ──────────────────────────────────────────────────
      tool(
        'devpilot_cli_tools_install',
        'Install a CLI tool by executing a shell command (e.g. "brew install ffmpeg", "pip install yt-dlp"). After the command succeeds, the tool is automatically detected and registered. This tool requires user permission before execution. After calling this tool, generate a bilingual description and call devpilot_cli_tools_add to save it.',
        {
          ...
          try {
            ...
            if (binCandidates.length === 0) {
              return {
                content: [{
                  type: 'text' as const,
                  text: `Command executed successfully but could not determine the binary name.\nOutput:\n${output.slice(0, 1000)}\n\nPlease use devpilot_cli_tools_add with the binary path to register it manually.`,
                }],
              };
            }
            ...
            if (binPath) {
              ...
              resultLines.push('');
              resultLines.push('Now please generate a bilingual description (zh/en) based on the help output above and call devpilot_cli_tools_add to save it.');
              ...
              return {
                content: [{
                  type: 'text' as const,
                  text: `Command executed but could not locate "${binName}" in PATH after installation.\nOutput:\n${output.slice(0, 1000)}\n\nThe tool may have been installed with a different binary name. Use "which" to find it, then call devpilot_cli_tools_add to register manually.`,
                }],
              };
            }
          } catch (error) {
      ...
      tool(
        ...
        'Register an already-installed CLI tool by its binary path, and optionally save its bilingual description and agent compatibility assessment. Use this after devpilot_cli_tools_install to save the generated description, or to register a tool the user has already installed. When providing descriptions, also assess the agent compatibility dimensions based on the --help output.',
        {
          ...
          try {
            ...
            if (descriptionZh) {
              ...
              if (helpOutput) {
                ...
                resultParts.push('');
                resultParts.push('Please generate a bilingual description (zh/en) based on the help output above and call devpilot_cli_tools_add with toolId to save it.');
              }
            }
            return {
              content: [{
      ...
      tool(
        ...
        async () => {
          try {
            ...
            lines.push('');
            lines.push('Use devpilot_cli_tools_update to update a specific tool.');
          ...
          try {
            ...
            if (!displayName || !packageName) {
              return {
                content: [{
                  type: 'text' as const,
                  text: `Tool "${toolId || toolName}" not found. Use devpilot_cli_tools_list to see available tools.`,
                }],
                isError: true,
              };
            }
