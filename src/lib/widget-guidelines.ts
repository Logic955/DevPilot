 ...
 * on demand via the `devpilot_load_widget_guidelines` in-process MCP tool.
 */
...
## Design specs
Call \`devpilot_load_widget_guidelines\` before your first widget to load detailed design specs.
...
/**
 * Creates an in-process MCP server that exposes `devpilot_load_widget_guidelines`.
...
export function createWidgetMcpServer() {
  return createSdkMcpServer({
    name: 'devpilot-widget-guidelines',
    version: '1.0.0',
    tools: [
      tool(
