/**
 * devpilot-dashboard MCP — in-process MCP server for dashboard widget management.
 *
...
- devpilot_dashboard_pin: Pin a widget to the dashboard. Provide widgetCode, title, dataContract, and data source info.
- devpilot_dashboard_list: List all pinned widgets with metadata.
- devpilot_dashboard_refresh: Read source data for a widget. Returns data for you to regenerate the widget.
- devpilot_dashboard_update: Save updated widget code, title, or data contract.
- devpilot_dashboard_remove: Remove a widget from the dashboard.
  ...
  return createSdkMcpServer({
    ...
    tools: [
      ...
      tool(
        ...
        'Read source data for a dashboard widget. For file sources, reads files and returns content. For MCP sources, tells you which tool to call. For CLI sources, returns the command for you to run via bash. After getting data, generate updated HTML and call devpilot_dashboard_update.',
        {
        ...
        async ({ widgetId }) => {
          ...
          if (ds.type === 'mcp_tool') {
            const result = [
              ...
              `Then generate updated widget HTML and call devpilot_dashboard_update.`,
              '',
          ...
          if (ds.type === 'cli') {
            const result = [
              ...
              ds.command,
              '```',
              `Then use the output to generate updated widget HTML and call devpilot_dashboard_update.`,
              '',
              `Original widget HTML (${widget.widgetCode.length} chars):`,
              '```',
              widget.widgetCode.slice(0, 8000),
          ...
          const result = [
            ...
            '',
            dataChanged
              ? 'Please generate updated widget HTML preserving the exact visual design, only updating data-driven content. Then call devpilot_dashboard_update with the new HTML.'
